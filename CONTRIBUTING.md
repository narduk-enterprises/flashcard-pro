...
```bash
git clone https://github.com/narduk-enterprises/flashcard-pro.git my-project
cd my-project
pnpm install
...
| ---------------------- | ---------- | -------------- |
| `flashcard-pro` | `dev`      | Shared secrets |
| `flashcard-pro` | `dev_auth` | `example-auth` |

**First-time setup for example-auth:**

```bash
cd apps/example-auth
doppler setup --project flashcard-pro --config dev_auth --no-interactive
```
